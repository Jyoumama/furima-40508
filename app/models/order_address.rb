class OrderAddress
  ...
  with_options presence: { message: "can't be blank" } do
    ...
    validates :postal_code, format: { with: /\A[0-9]{3}-[0-9]{4}\z/, message: 'is invalid. Enter it as follows (e.g. 123-4567)' },
                            length: { is: 8, message: 'is the wrong length (should be 8 characters)' }
    validates :prefecture_id, numericality: { other_than: 0, message: "can't be blank" }
    validates :city
    validates :addresses
    validates :phone_number, numericality: { only_integer: true, message: 'is invalid. Input only number' }, format: { with: /\A\d{10,11}\z/, message: 'is too short' },
    length: { in: 10..11, message: 'is invalid' }
    validates :token
  end
